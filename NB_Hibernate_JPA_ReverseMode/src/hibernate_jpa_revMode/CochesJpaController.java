...
import hibernate_jpa_revMode.exceptions.NonexistentEntityException;
...
public class CochesJpaController implements Serializable {
    ...
    private List<Coches> findCochesEntities(boolean all, int maxResults, int firstResult) {
        ...
        try {
            Query q = em.createQuery("select object(o) from Coches as o");
            if (!all) {
    ...
    public int getCochesCount() {
        EntityManager em = getEntityManager();
        try {
            Query q = em.createQuery("select count(o) from Coches as o");
